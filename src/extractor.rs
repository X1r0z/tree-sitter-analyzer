...
mod tests {
    ...
    #[test]
    fn java_super_calls_resolve_to_nearest_parent_method() {
        let (_dir, file) = write_temp_file(
            "Demo.java",
            r#"
class Demo {
    public static void main(String[] args){ new Sub().method(); }
}
class Super { void method() { } }
class Middle extends Super { void method() { } }
...
"#,
        ...
        let callees = extractor.find_function_callees("method", None);

        assert!(callees.iter().any(|(callee, _)| callee == "Middle.method"));
        assert!(!callees.iter().any(|(callee, _)| callee == "super.method"));
    }

    #[test]
    fn javascript_local_new_expression_receiver_resolves_to_class_method() {
        let (_dir, file) = write_temp_file(
            "main.js",
            r#"
class C {
...
"#,
        ...
        let callees = extractor.find_function_callees("func", Some("A"));
        let callers = extractor.find_function_callers("func", Some("B"), false);

        assert!(callees.iter().any(|(callee, _)| callee == "B.func"));
        assert!(!callees.iter().any(|(callee, _)| callee == "b.func"));
        assert!(callers.iter().any(|(caller, _)| caller == "func"));
    }
}
