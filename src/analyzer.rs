...
mod tests {
    ...
    use crate::nodes::FunctionParamInfo;

    fn analyze_functions(ext: &str, source: &str) -> Vec<crate::nodes::FunctionInfo> {
        let dir = tempdir().unwrap();
        let path = dir.path().join(format!("sample.{ext}"));
        fs::write(&path, source).unwrap();

        let mut analyzer = CodeAnalyzer::new(path.to_str().unwrap()).unwrap();
        analyzer.functions()
    }

    #[test]
    fn extracts_python_function_params() {
        let functions = analyze_functions(
            "py",
            "def greet(name: str, times=1, *args, **kwargs):\n    return name\n",
        );
    ...
    #[test]
    fn extracts_typescript_method_params() {
        let functions = analyze_functions(
            "ts",
            "class Greeter {\n  hello(name: string, count = 1, ...rest: string[]) {}\n}\n",
        );
    ...
    #[test]
    fn extracts_java_function_params() {
        let functions = analyze_functions(
            "java",
    ...
    #[test]
    fn extracts_go_function_params() {
        let functions = analyze_functions(
            "go",
            "package main\n\nfunc greet(name string, count int, values ...string) {}\n",
        );
