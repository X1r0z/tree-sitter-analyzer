...
"#;
    ...
    #[test]
    fn property_graphs_are_disambiguated_by_receiver_type() -> anyhow::Result<()> {
        let temp = tempdir()?;
        fs::write(temp.path().join("sample.py"), PYTHON_PROPERTY_COLLISION)?;

        let analyzer =
            ProjectAnalyzer::new_with_language(temp.path().to_str().unwrap(), Some("python"))?;

        let alpha_graphs =
            analyzer.find_graphs("value", Some("Alpha"), GraphDirection::Backward, 1)?;
        ...
        let beta_graphs =
            analyzer.find_graphs("value", Some("Beta"), GraphDirection::Backward, 1)?;
