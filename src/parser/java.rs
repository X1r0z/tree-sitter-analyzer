...
mod tests {
    ...
    #[test]
    fn extracts_java_target_signatures_for_class_method_and_parameter_annotations() {
        let dir = tempdir().unwrap();
        let file_path = dir.path().join("Demo.java");
        fs::write(
            &file_path,
            r#"@ClassAnno
...
"#,
        ...
        let annotations = parser.extract_java_annotations();

        let class_annotation = annotations
            .iter()
            .find(|item| item.name == "ClassAnno")
            .unwrap();
        assert_eq!(
            class_annotation.target_signature,
            "@ClassAnno\npublic class Demo extends Base"
        );

        let method_annotation = annotations
            .iter()
            .find(|item| item.name == "RequestMapping")
            .unwrap();
        assert_eq!(
            method_annotation.target_signature,
            "@RequestMapping\npublic String hello(\n        @Parameter String name,\n        int age\n    )"
        );

        let parameter_annotation = annotations
            .iter()
            .find(|item| item.name == "Parameter")
            .unwrap();
        assert_eq!(
            parameter_annotation.target_signature,
            "public String hello(\n        @Parameter String name,\n        int age\n    )"
        );
    }
}
