                ...
                if name_node.kind() == "identifier" {
                    ...
                    if value_node.kind() == "identifier" {
                        ...
                        let targets = (0..value_node.named_child_count())
                            ...
                            .collect::<Vec<_>>();
                        if let Some((name, targets)) = add_alias(&mut aliases, Some(name), targets)
                        {
                            events.push(JsAliasEvent {
                                start_byte: node.start_byte(),
                                name,
            ...
            if let (Some(left_node), Some(right_node)) = (left_node, right_node) {
                if right_node.kind() == "identifier" {
                    ...
                    if let Some(targets) = aliases.get(&iterable).cloned() {
                        if let Some((name, targets)) = add_alias(&mut aliases, loop_var, targets) {
                            events.push(JsAliasEvent {
                                start_byte: node.start_byte(),
                                name,
                                targets,
            ...
            if let Some(left) = node.child_by_field_name("left") {
                if left.kind() == "member_expression" {
                    ...
                    if let (Some(obj), Some(prop)) = (obj, prop) {
                        if parser.node_text_eq(obj, "this") {
                            let name = parser.node_text(prop);
                            if !name.is_empty() && seen.insert(name.clone()) {
                                fields.push(FieldInfo {
                                    name,
    ...
    match value_node.kind() {
        "call_expression" => {
            ...
            let is_supported_factory = matches!(
                ...
                parser.node_text(function).as_str(),
            ...
            if matches!(sub.kind(), "extends_clause" | "implements_clause") {
                for k in 0..sub.child_count() {
                    ...
                    if grandchild.is_named() {
                        collect_super_class_names_from_expression(
                            parser,
                            grandchild,
                            super_classes,
                            seen,
                ...
                collect_super_class_names_from_expression(parser, sub, super_classes, seen);
            }
        }
    }
}
    ...
    match node.kind() {
        ...
        "expression_with_type_arguments" => {
            if let Some(expr) = node.child_by_field_name("expression") {
                collect_super_class_names_from_expression(parser, expr, super_classes, seen);
                return;
            }
            for i in 0..node.named_child_count() {
                if let Some(child) = node.named_child(i as u32) {
                    collect_super_class_names_from_expression(parser, child, super_classes, seen);
                    return;
                }
            }
        }
        "call_expression" => {
            if let Some(function) = node.child_by_field_name("function") {
                collect_super_class_names_from_expression(parser, function, super_classes, seen);
            }
        }
        _ => {
            for i in 0..node.named_child_count() {
